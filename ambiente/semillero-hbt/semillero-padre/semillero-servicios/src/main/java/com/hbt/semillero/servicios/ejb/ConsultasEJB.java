...
public class ConsultasEJB implements IConsultasEjbLocal {
	...
	/**
	 * metodo para consultar un vehiculo por medio de una linea
	 * @see com.hbt.semillero.servicios.interfaces.IConsultasEjbLocal#consultarVehiculos(long)
	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public List<VehiculoDTO> consultarVehiculos(long idLinea) {
		StringBuilder consulta = new StringBuilder("Select ve FROM Vehiculo ve WHERE 1=1 ");
		
		Map<String, Object> parametros = new HashMap<>();
		
		if (idLinea != 0) {
			System.out.println("idLinea" + idLinea);
			consulta.append(" and v  e.linea =:idLinea");
			parametros.put("linea", idLinea);
		}
		Query query = em.createQuery(consulta.toString());
	...
	/**
     * Metodo utilizado para crear vehuculos, este es un post
     * @param persona
     */
